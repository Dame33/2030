...
public final class Lab1 {
	...
	public static int product(int start, int end) {
		if (start > end) { // basis step
			return 1;
			}
		else { // when input does not meet basis step, skips to else statement.
			return start * product( start + 1, end ); //returns input start, and recalls product function with new inputs start + 1, end.
		}
	}
	/**
	...
	public static String makeString(char first, int n) {
		if (n == 0){ // basis step, when n == 0 (minimum possible value)
			return ("") ; //returns an blank space
		}
		else { // if input does not meet basis step, skips to else statement.
			return first + makeString(first, n - 1); 
			/**
			 * returns inputted char first. 
			 * String is attached using '+',
			 * function makeAString is called again with new inputs first, n-1.
			 * thus allowing for function to loop until the end of the case.
			 */
		}
	}
	...
	public static String writeChars(int n) {
		...
		else if (n % 2 == 1) { // checks if input is odd
			if (n == 1) { 
				return ("*");
			}
			else {
				return "<" + writeChars(n-2) + ">"; // adds < > and recalls the function with a value 2 less.
			}
		}
		else if (n % 2 == 0) { // checks if input is even
			if (n == 2) {
			return ("**");
			}
			else {
		...
		/**
		 * returns function with new input of n - 2
		 * this allows for the code to check one more time to see if it has reached our original basis step
		 * Thus allowing for the code to stop itself when it has reached the lowest possible value.
		 * 
		 */
	}
	...
	public static String getSubstring(String str, char open, char close) {
		...
		if(openIndex == -1) { //basis step
			return null; // returns null
		}
		else if (closeIndex == -1) { // basis step
			return null; // returns null
		}
		return str.substring(str.indexOf(open) + 1, str.lastIndexOf(close));
		/**
		 * the function str.substring is called with inputs of str.indexOf(open) + 1 and str.lastIndexOf(close)
		 * Thus allowing for the code to check through every word
		 */
	}
	...
	public static String decimalToBinary(int value) {
		if (value == 0) { // basis step
			return "0"; // returns 0
		}
		else if (value == 1) { // basis step
			return "1"; // returns 1
		}
		else if (value % 2 == 1) { // if remainder is 1
			value = value/2; //reassigns var value with new information
			return decimalToBinary(value) + "1"; // recalls the function, and attaches it with string 1
		}
		else if (value % 2 == 0) { // if remainder is 0
			value = value/2; //reassigns var value with new information
			return decimalToBinary(value) + "0"; // recalls the function, and attaches it with string 1
		}
		return "";
		/**
		 * upon going through the basis steps and all else if statements
		 * returns a " ". as a means to stop the code from continuously looping
		 */
	}
